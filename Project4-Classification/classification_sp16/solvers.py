...
class GradientDescentSolver(Solver):
	...
	for i in range(len(param_vars)):
		updated_param = param_vars[i] - (self.learning_rate * grad_tensors[i])
		updates.append((param_vars[i],updated_param))
	...
	for i in range(len(param_vars)):
		...
		updated_param = param_vars[i] + updated_value
		updates.append((param_vars[i], updated_param))		
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []	
	#model.input_ph = tf.constant([input_train_data])
	#target_ph = tf.constant([target_train_data])	
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
	    train_loss = session.run(loss_tensor, feed_dict = {model.input_ph: train_data[0], target_ph: train_data[1]})
	    session.run(update_ops, feed_dict = {model.input_ph: train_data[0], target_ph: train_data[1]})
	    val_loss = session.run(loss_tensor, feed_dict = {model.input_ph: val_data[0], target_ph: val_data[1]})

	    #session.run(update_ops, feed_dict = {model.input_ph: input_val_data, target_ph: target_val_data})
...
class StochasticGradientDescentSolver(GradientDescentSolver):
	...
	train_miniBatch = MinibatchIndefinitelyGenerator(train_data,1,self.shuffle)
	val_miniBatch = MinibatchIndefinitelyGenerator(val_data,1,self.shuffle)	
	    ...
	    val_input, val_target = val_miniBatch.next()
	    train_input, train_target = train_miniBatch.next()
	    train_loss = session.run(loss_tensor, feed_dict = {model.input_ph: train_input, target_ph: train_target})
	    session.run(update_ops, feed_dict = {model.input_ph: train_input, target_ph: train_target})
	    val_loss = session.run(loss_tensor, feed_dict = {model.input_ph: val_input, target_ph: val_target})
            #util.raiseNotDefined()
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
	...
	train_miniBatch = MinibatchIndefinitelyGenerator(train_data, self.batch_size ,self.shuffle)
	val_miniBatch = MinibatchIndefinitelyGenerator(val_data, self.batch_size ,self.shuffle)	
