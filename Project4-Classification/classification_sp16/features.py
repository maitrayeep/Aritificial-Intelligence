...
def enhancedFeatureExtractor(datum):
    ...
    for i in range(width):
		...
		if datum[i,j] == 0 and (i,j) not in visited_nodes:
			...
			queue.push((i, j))
        		...
        		while not queue.isEmpty():
	    			...
	    			x = val[0]
	    			y = val[1]
				if x > 0 and datum[x-1,y] == 0 and (x-1, y) not in visited_nodes:
                			visited_nodes.add((x-1, y))
                			queue.push((x-1, y))


	    			if y > 0 and datum[x,y-1] == 0 and (x, y-1) not in visited_nodes:
                			visited_nodes.add((x, y-1))
                			queue.push((x, y - 1))

            
	    			if x + 1 < width and datum[x+1,y] == 0 and (x+1, y) not in visited_nodes:
                			visited_nodes.add((x+1, y))
                			queue.push((x+1, y))

	    			if y + 1 < height and datum[x,y+1] == 0 and (x,y+1) not in visited_nodes:
                			visited_nodes.add((x,y+1))
                			queue.push((x,y+1))
    
    
    if index == 1:
        new_feature = np.array([1, 0, 0])
    elif index == 2:
        new_feature = np.array([0, 1, 0])
    elif index == 3:
        new_feature = np.array([0, 0, 1])
    else:
	new_feature = np.array([0, 0, 0])

    return np.concatenate((features, new_feature))
